...
const fetchRssData = () => {
  ...
  axios
      .get(`https://api.allorigins.win/get?url=${encodeURIComponent(rssUrl)}`, {timeout: 13000})
      .then((response) => {
        ...
        items.forEach((item) => {
          ...
          if (category === 'Sport') {
            newRssItems.push({
              ...
              image: imageUrl ? imageUrl.getAttribute('url') : '/images/the-stadium.jpg',
...
<template>
  <div class="max-w-6xl px-2 py-6 sm:px-6 lg:px-0 lg:py-14 mx-auto">
    ...
    >
      <LeftBackIcon class="w-7 h-7 text-teal-600 hover:text-teal-500 cursor-pointer"/>
    </button>
    <h1 class="text-center pb-2 font-bold md:text-3xl text-2xl text-teal-500" :class="{ 'hidden': $route.href === '/' }">Yangiliklar</h1>
    <p class="text-center pb-4 max-w-3xl mx-auto text-lg" :class="{ 'hidden': $route.href === '/' }">Futbol olamiga oid eng so'nggi yangiliklar</p>
    <!--    NewsSection style-->
    <div v-if="isHome">
      <div class="flex justify-between max-w-5xl items-center md:mx-auto text-teal-600 font-bold px-2 mt-4">
      ...
      >
        <div v-if="isLoading" v-for="skeleton in 6" role="status"
        ...
        <a v-else :href="item.link" v-for="item in rssItems" class="group ml-4 relative block w-96 h-60">
          <div
            ...
            <img class="w-full h-full absolute top-0 left-0 object-cover" src="/images/the-stadium.jpg" alt="Image Description">
          </div>
          <div class="absolute top-0 inset-x-0 z-10">
            <div class="p-2 flex flex-col h-full sm:p-3">
              <div class="flex items-center">
                <div class="sm:ml-2 bg-white opacity-80 p-1 rounded-lg">
                  <h4 class="font-semibold text-sm text-teal-600">
                    Sport
                  </h4>
          ...
          <div class="absolute bottom-0 w-full z-10">
            <div class="h-full p-3 sm:p-5">
              <h3 class="md:text-lg sm:text-xl font-semibold text-white group-hover:text-gray-300 line-clamp-2">
                {{ item.title }}
              </h3>
    ...
    <div v-else class="grid pb-8 lg:grid-cols-2 lg:gap-y-5 gap-10">
      <div v-if="isLoading" v-for="skeleton in 6" class="space-y-8 animate-pulse md:space-y-0 md:space-x-8 md:flex md:items-center">
        <div class="flex items-center justify-center w-full h-48 bg-gray-400 rounded sm:w-96">
          <ImageIcon class="w-10 h-10 text-gray-100"/>
        </div>
        <div class="w-full">
      ...
      <a v-else :href="item.link" v-for="(item, index) in rssItems" :key="index"
         class="group rounded-xl overflow-hidden shadow-md border border-gray-200 p-2">
        <div class="sm:flex">
          <div class="flex-shrink-0 relative rounded-xl overflow-hidden w-full sm:w-60 md:h-44 h-64">
            <img
                ...
                src="/images/the-stadium.jpg" alt="image" v-if="item.image">
          </div>
          <div class="grow mt-4 sm:ml-6 px-2 sm:px-0">
            <h3 class="text-sm font-semibold text-gray-800 group-hover:text-gray-600 line-clamp-2">
              {{ item.title }}
            </h3>
            ...
            <div class="flex justify-between items-center mt-4">
              ...
              <p class="inline-flex items-center gap-x-2 md:pr-2 text-teal-600 decoration-2 hover:underline font-medium">
                Read more
                <RightIcon class="w-4 h-4 group-hover:translate-x-1 duration-300"/>
              </p>
            </div>
          </div>
        </div>
