...
const validateStartTime = () => {
  ...
  if (selectedStartTime < currentTime) {
    showWarningToast(
      "Boshlanish vaqti joriy vaqtdan keyin bo'lishi mumkin emas."
    );
    return false;
  }
  return true;
...
const validateCustomerName = () => {
  ...
  if (
    nameParts.length < 1 ||
    nameParts.some((part) => !/^[a-zA-Z]+$/.test(part))
  ) {
    showWarningToast(
      "Iltimos, ismingiz va familiyangizni faqat harflardan foydalanib to'liq kiriting!"
    );
    return false;
  }
  return true;
  ...
  <div class="px-2 my-8">
    <div class="flex justify-around mb-5">
      <button @click="goBackToPreviousStep">
        <LeftBackIcon
          class="text-teal-600 cursor-pointer w-7 h-7 hover:text-teal-500"
        />
      </button>
      <h1 class="flex-grow text-lg font-bold text-center text-teal-600">
        Stadion band qilish
      </h1>
    </div>
    <div class="mx-4">
      <div class="overflow-hidden bg-gray-200 rounded-full">
        <div
          ...
          class="h-2 transition-all bg-teal-500 rounded-full"
        ></div>
      </div>
      <ol class="grid grid-cols-3 mt-4 text-sm font-medium text-gray-500">
        <li
          class="flex items-center justify-start text-teal-500 sm:gap-1.5"
          :class="{ 'text-teal-500': currentStep >= 1 }"
        >
        ...
        <li
          ...
          :class="{ 'text-teal-500': currentStep >= 2 }"
        >
        ...
        <li
          ...
          :class="{ 'text-teal-500': currentStep >= 3 }"
        >
    ...
    <div v-if="currentStep === 1" class="flex flex-col w-full mt-3 md:mx-auto">
      <div class="flex flex-wrap justify-center mt-1">
        ...
        <div class="flex p-4">
          ...
          <div class="flex flex-col justify-center p-2">
            <label for="startTime" class="px-1 text-sm font-bold text-teal-600"
          ...
          <div class="flex flex-col justify-center p-2">
            <label
              for="match-time"
              class="block text-sm font-bold text-teal-600"
              >O'yin vaqti</label
            >
            <select
              id="match-time"
        ...
        <div class="w-full p-4">
          <div class="max-w-3xl md:mx-auto">
            <label for="client-name" class="text-sm font-bold text-teal-600"
              >Band qiluvchi shaxs:</label
            >
            <input
              id="client-name"
              v-model="clientName"
      ...
      <div class="flex justify-end mt-5">
        <button
          @click="goToNextStep"
          class="px-4 py-2 font-medium text-white bg-teal-500 rounded-lg"
        >
          Keyingi
        </button>
      </div>
    ...
    <div v-if="currentStep === 2" class="mt-4">
      ...
      <div class="flex justify-between mx-2 mt-14">
        <button
          ...
          class="px-4 py-2 mt-4 font-medium text-white bg-gray-500 rounded-lg"
        >
          Orqaga
        </button>
        <button
          @click="goToThirdStep"
          class="px-4 py-2 mt-4 font-medium text-white bg-teal-500 rounded-lg"
        >
          Keyingi
        </button>
      </div>
    ...
    <div class="max-w-3xl px-2 mt-6 md:mx-auto" v-if="currentStep === 3">
      <div
        class="relative flex flex-col w-full text-gray-700 bg-white border rounded-lg"
      >
        <div
          class="md:h-80 h-60 relative bg-center bg-cover mx-3 mt-4 overflow-hidden text-white shadow-lg rounded-lg"
          :style="{
        ...
        <div class="p-3">
          <div class="flex items-center py-2">
            ...
            <h1 class="px-1 text-sm font-medium">
