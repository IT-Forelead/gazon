...
<template>
  <div class="fixed z-40 w-full -translate-x-1/2 left-1/2 bottom-0">
    <div class="h-32 tab-style mx-auto">
      <ul class="flex absolute bottom-0 w-full h-[50px] justify-center bg-teal-500">
        <li
            v-for="tab in tabs"
            :key="tab.id"
            :class="{ active: isActive(tab.id) }"
            class="pb-1 top-[1px] w-[60px] h-[50px] flex items-center justify-center bg-teal-500 mx-4 cursor-pointer relative transition-all duration-300 rounded-tl-full rounded-tr-full"
